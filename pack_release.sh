...
# PDF-SplitExport Release-Pack-Script
# Erstellt Release-Archive fuer Windows, Linux und macOS.
# Erwartete Dateien in `dist/`:
#   PDF-SplitExport.exe   -> Windows Build
#   PDF-SplitExport       -> Linux Build
#   dist/*mac*.zip        -> macOS Build-ZIP (GitHub Actions)
# Ausgabe in `release/` mit heutigem Datum (oder RELEASE_DATE):
#   release/PDF-SplitExport-Windows-...zip
#   release/PDF-SplitExport-Linux-....tar.gz
#   release/PDF-SplitExport-macOS-...zip
...
copy_common_files() {
    ...
    [[ -f "$ROOT_DIR/README_LOCAL_DE.md" ]] && cp "$ROOT_DIR/README_LOCAL_DE.md" "$target_dir/"
...
# macOS-ZIP normalisieren: Root-Ordner aufloesen, .app bleibt erhalten.
